package com.t_epc.reader.server;
...
import com.icomp.common.utils.LocalBroadcastManager;
import com.t_epc.reader.CMD;
import com.t_epc.reader.ERROR;
import com.t_epc.reader.HEAD;
import com.t_epc.reader.MessageTran;
...
import com.t_epc.utils.StringTool;
import com.t_epc.utils.Tools;
...
public class ReaderHelper {
        ...
        if (mReader == null) {
            ...
            mReader = new ReaderBase(in, out) {
                ...
                public void reciveData(byte[] btAryReceiveData) {
                    ...
                    itent.putExtra("log", strLog);
                    //接收的数据记录入磁盘
                    Loger.disk_log(type.toString(), "read：" + strLog, "M10_U8");
                ...
                public void sendData(byte[] btArySendData) {
                    ...
                    itent.putExtra("log", strLog);
                    //发送的数据记录入磁盘
                    Loger.disk_log(type.toString(), "write：" + strLog, "M10_U8");
                    mLocalBroadcastManager.sendBroadcast(itent);
                }
            };
        }
    ...
    private void processReadTag(MessageTran msgTran) {
        ...
        if (btAryData.length == 0x01) {
            ...
            String strLog = strCmd + "Fail，Fail Reason： " + strErrorCode;
    ...
    private void processRealTimeInventory(MessageTran msgTran) {
        ...
        if (btAryData.length == 0x01) {        //本次获取因为错误结束
            ...
            //  runLoopInventroy(); //TODO 是否是循环？
        } else {
            m_nTotal++;
            int nLength = btAryData.length;
            int nEpcLength = nLength - 4;
            // TODO  rfid就是你
            String strEPC = StringTool.byteArrayToString(btAryData, 3, nEpcLength).replace(" ", "");
            //	Log.i("rfid",strEPC);
            Constat.RFID_CODE_STRING = strEPC;
            Log.i("rfid_no_speat", Constat.RFID_CODE_STRING);
            if (Constat.RFID_CODE_STRING != null) {
                return;
            } else {
                runLoopInventroy();
            }
